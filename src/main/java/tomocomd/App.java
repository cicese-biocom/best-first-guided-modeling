...
public class App {
    ...
    private static void parseOptions(String[] args) {
        ...
        options.addOption("x", "external", true, "input, external folder with several external datasets, csv format");
        options.addOption("e", "endpoint", true, "property target");
        options.addOption("c", "classification", false, "it is a classification problem, if it is a regression problem not set this option");
        Option opt = new Option("m", "models", true, "List with the desirable strategies, " +
        ...
        List<AbstractClassifier> classifierList = cmd.hasOption("m") ? BuildClassifierList.getclassifierList(cmd.getOptionValues("m"), isClassification) : Collections.emptyList();
        ...
        try {
            if (cmd.hasOption('c')) {
                List<ClassificationOptimizationParam> params = Objects.isNull(tunePath) ?
                        new ArrayList<>(Collections.singletonList(ClassificationOptimizationParam.MCC_TRAIN)) :
                        new ArrayList<>(Collections.singletonList(ClassificationOptimizationParam.MCC_MEAN));
                ...
                InitSearchClassificationModel initSearchClassificationModel = new InitSearchClassificationModel(
                        newData, trainFile.getAbsolutePath(), newTest, pathTune, extFolderPath, act, classifierList,
                        asSearches, params);

                initSearchClassificationModel.startSearchModel();

            } else {
                List<RegressionOptimizationParam> params = Objects.isNull(tunePath) ?
                ...
                String pathTune = Objects.nonNull(tunePath) ? tunePath.getAbsolutePath() : null;
                InitSearchRegressionModel initSearchRegressionModel = new InitSearchRegressionModel(data,
                        trainFile.getAbsolutePath(), tune, pathTune, extFolderPath, act, classifierList, asSearches,
                        params);
