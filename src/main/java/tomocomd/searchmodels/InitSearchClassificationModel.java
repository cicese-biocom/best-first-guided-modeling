...
public class InitSearchClassificationModel extends AInitSearchModel {
    ...
    public InitSearchClassificationModel(String train, String tune, String folderExtSdf, String act,
    ...
    public InitSearchClassificationModel(Instances train, String csvFile, Instances test, String tuneSdf,
                                         String folderExtSdf, String act, List<AbstractClassifier> classifierList,
                                         List<ASSearch> searchList, List<ClassificationOptimizationParam> params) {
        ...
        for (AbstractClassifier clas : classifierList) {
            for (ASSearch aSSearch : searchList) {
                for (ClassificationOptimizationParam opt : params) {
                    try {
                        selectionClassificationOne(dataTrainThread, csvFile, dataTestThread, tuneSdf, extInsts, opt, clas, aSSearch, cantModels, models);
    ...
    private static void selectionClassificationOne(String pathCSV, String test, List<String> externals, String act,
                                                   ClassificationOptimizationParam opt, AbstractClassifier classifier,
                                                   ASSearch aSSearch, long idModels, List<ClassificationModelInfo> models) throws AExOpDCSException {
        ...
        SearchClassificationModel search = new SearchClassificationModel(idModels, pathCSV, test, externals, pathCSV + "_models.csv",
        ...
        try {
            ...
            throw AExOpDCSException.ExceptionType.BUILDING_MODEL_EXCEPTION.get("Problems building classification models", ex);
        }
    }

    private static void selectionClassificationOne(Instances data, String pathTrain, Instances test, String pathTest,
                                                   List<String> externals, ClassificationOptimizationParam opt,
                                                   AbstractClassifier classifier, ASSearch aSSearch, long idModels,
                                                   List<ClassificationModelInfo> models) throws AExOpDCSException {
        ...
        SearchClassificationModel search = Objects.nonNull(test) ?
                new SearchClassificationModel(idModels, data, pathTrain, inst, pathTest, externals, nameFile,
                        cIdx, classifier, opt, models) :
                new SearchClassificationModel(idModels, data, pathTrain, pathTest, externals, nameFile,
                        cIdx, classifier, opt, models);
