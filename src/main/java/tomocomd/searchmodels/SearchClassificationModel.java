...
public class SearchClassificationModel extends ASearchModel {
    ...
    public SearchClassificationModel(long mId, String trainPath, String testPath, List<String> externalTestPath, String pathToSave,
                                     int classAct, AbstractClassifier cls, ClassificationOptimizationParam oP,
                                     List<ClassificationModelInfo> listClassModelInfo) throws AExOpDCSException {
    ...
    public SearchClassificationModel(long mId, Instances data, String trainPath, Instances test,  String testPath,
                                     List<String> externalTestPath, String pathToSave, int classAct,
                                     AbstractClassifier cls, ClassificationOptimizationParam oP,
                                     List<ClassificationModelInfo> listClassModelInfo) throws AExOpDCSException {
    ...
    public SearchClassificationModel(long mId, Instances data, String trainPath, String testPath,
                                     List<String> externalTestPath, String pathToSave, int classAct,
                                     AbstractClassifier cls, ClassificationOptimizationParam oP,
                                     List<ClassificationModelInfo> listClassModelInfo) throws AExOpDCSException {
    ...
    protected double getEvaluation(Instances train, Instances internalTest, List<Instances> externalTests) throws AExOpDCSException{
        ...
        try {
            ...
            if (internalTest == null) {
                res = buildLineInfoModel(trainACC, trainSen, trainSpe, trainMCC, 0, 0, 0, 0, resExternal, mdNames,clasTmp);
            } else {
                res = buildLineInfoModel(trainACC, trainSen, trainSpe, trainMCC, testACC, testSen, testSpe, testMCC, resExternal, mdNames,clasTmp);
            }
    ...
    private void addLineModelToFile(String res) throws AExOpDCSException {
        ...
        if (!(new File(pathToSave).exists())) {
            String head;
            if (testPath == null)
                head = "classifier,id,size,ACC_CV,SEN_CV,SEP_CV,MCC_CV";
            else
            ...
            if (!externalTestPath.isEmpty()) {
                for (String nameExt : externalTestPath) {
                    ...
                    String headPart = String.format("ACC_%s,SEN_%s,SPE_%s,MCC_%s", singleName, singleName, singleName, singleName);
                    if (extHead.isEmpty()) {
                        extHead = headPart;
                    } else {
    ...
    private String buildLineInfoModel(double trainACC, double trainSen, double trainSpe,
                                      double trainMCC, double testACC, double testSen, double testSpe,
                                      double testMCC, String resExternal, Set<String> mdNames, AbstractClassifier clasTmp) throws AExOpDCSException {
        String res;
        String clasName = getClassifierName(clasTmp);

        res = String.format("%s,%d,%d,%.5f,%.5f,%.5f,%.5f", clasName,
                modelId, mdNames.size(), trainACC, trainSen, trainSpe, trainMCC);
        ...
        listMInfo.add(new ClassificationModelInfo(classifier, mdNames.toArray(new String[0]), "Ext_" + clasName, "", mdNames.size(), ++modelId, trainACC, trainSen, trainSpe, trainMCC,
                testACC, testSen, testSpe, testMCC));
        return res;
    }
    ...
    private String evaluateExternals(Instances instTrain, List<Instances> extInst, Classifier[] copies) throws AExOpDCSException {
        ...
        for (Instances ext : extInst) {
            ...
            try {
                ...
                throw AExOpDCSException.ExceptionType.BUILDING_MODEL_EXCEPTION.get("Problems evaluating model on external sets", e);
            }
            ...
            if (resExternal.isEmpty()) {
                ...
                resExternal = String.format("%s,%.5f,%.5f,%.5f,%.5f", resExternal, extACC, extSen, extSpe, extMCC);
            }
        }
        return resExternal;
    }
