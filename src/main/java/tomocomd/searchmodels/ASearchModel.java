...
public abstract class ASearchModel extends ASEvaluation implements SubsetEvaluator {
    ...
    protected ASearchModel(String trainPath, String testPath, String pathToSave, int classAct, long modelId,
                        List<String> externalTestPath, AbstractClassifier classifier) throws AExOpDCSException {
        ...
        try {
            ...
            throw AExOpDCSException.ExceptionType.CLASSIFIER_LOAD_EXCEPTION.get(ERROR_MSG, e);
        }
    }

    protected ASearchModel(Instances trainTotal, String trainPath, String testPath, String pathToSave, int classAct, long modelId,
                           List<String> externalTestPath, AbstractClassifier classifier) throws AExOpDCSException {
        this.trainPath = trainPath;
    ...
    protected ASearchModel(Instances trainTotal, String trainPath,Instances testTotal, String testPath, String pathToSave, int classAct, long modelId,
                           List<String> externalTestPath, AbstractClassifier classifier) throws AExOpDCSException {
        this.trainPath = trainPath;
        this.trainTotal = new Instances(trainTotal);
        this.testPath = testPath;
    ...
    @Override
    public double evaluateSubset(BitSet bitset) throws AExOpDCSException {

        int[] idx = IntStream.concat(Arrays.stream(new int[]{0}), IntStream.of(bitset.stream().sorted().toArray())).toArray();
        if (idx.length == 1)
            return 0;
        ...
        if (idx.length < trainTotal.numAttributes()) {
            ...
            if (Objects.nonNull(testTotal))
                test = remove(testTotal, idx);
    ...
    protected abstract double getEvaluation(Instances train, Instances internalTest, List<Instances> externalTests) throws AExOpDCSException;

    protected Instances readCsv(String path, int act) throws AExOpDCSException {
    ...
    protected Instances remove(Instances data, int[] pos) throws AExOpDCSException {
        ...
        try {
            ...
            throw AExOpDCSException.ExceptionType.LOAD_DATASET_EXCEPTION.get(String.format("Problems loading dataset:%s", data.relationName()), ex);
        }
    }
